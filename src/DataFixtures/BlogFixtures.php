...
class BlogFixtures extends Fixture
{
    public function load(ObjectManager $manager): void
    {
        ...
        // on utilise le site picsum. photos pour générer des photos (avec un indice afin que cette photo soit tjs la même ensuite)
        ...
        $profile = (new Profile())->setPicture("https://picsum.photos/360/360?image=".$i)
                                  // le ($i+99) permet d'avoir une photo différente du $i
        ...
        for ($i=0;$i< 5 ; $i++) {
            ...
            $category = (new Category())->setName($faker->sentence (1))
                                        ->setDescription($faker->paragraph())
                                        ->setImageUrl("https://picsum.photos/359/360?image=".($i+200))
                                        ->setCreateAt ($dateC);
            $categories[] = $category;
        ...
        for ($i=0;$i< 100; $i++) {
            ...
            $article = (new Article())->setTitle($faker->sentence (2))
                                    ->setContent($faker->text(79))
                                    ->setImageUrl("https://picsum.photos/359/360?image=".($i+300))
                                    ->setCreateAt($dateArt)
                                    // tirage aléa d'un auteur/user pour cet article 
                                    //->setAuthor($users [rand (-1, count($users)-1)]) 
                                    // tirage aléa d'une categorie pour cet article 
